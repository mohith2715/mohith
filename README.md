...
This project is designed to run locally with zero external dependencies.
...
2.  **Team Based Division**: Creation of teams and ideas specific to teams with restricted access.
3.  **UI Componentization**: Migrate the vanilla HTML/JS to a framework like React or Vue for better maintainability.
4.  **User Profile/ Management Page**: Custom user profile page with thier information, ideas and picture
5.  **Reworked Rewierer System**: Reviwer role assigning could be reworked to invite only or uesr-request methods.
6.  **Admin Dashboard**: Create a dedicated view for Admins to manage users, reset passwords, and configure the application (e.g., changing the Reviewer Code dynamically).
7.  **Notifications**: Implement email notifications for submitters when their idea status changes.
